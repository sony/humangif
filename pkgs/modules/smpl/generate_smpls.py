...
import pyrender
import torch
from modules.smpl import DETECTRON2_MODEL_PATH, HMR2_DEFAULT_CKPT
...
def load_image(img_cv2, model_cfg, detector):
...
def generate_smpl(image_file: str, output_dir: str, device, model=None, model_cfg=None, detector=None):
    if not model or not detector:
        model, model_cfg, detector = load_model(device)

    img_cv2 = cv2.imread(image_file)
    img_fn, _ = os.path.splitext(os.path.basename(image_file))
    dataloader = load_image(img_cv2, model_cfg, detector)
    
    os.makedirs(output_dir, exist_ok=True)

    for batch in dataloader:
        batch = recursive_to(batch, device)
        results_dict_for_rendering, _ = predict_smpl(batch, model, model_cfg)
        np.save(os.path.join(output_dir, f"{img_fn}.npy"), results_dict_for_rendering)
