...
def import_smpls_group(smpls_group_path):
    ...
    smpls, cams = [], []
    results = {}
    smpl_file_lst = sorted(os.listdir(smpls_group_path))
    for smpl_file in smpl_file_lst:
        smpl_data = np.load(f'{smpls_group_path}/{smpl_file}', allow_pickle=True).item()
        cams.append(smpl_data["cam_t"][0])
        smpls.append(smpl_data["smpls"])
    results["smpl"] = smpls
    results["camera"] = cams
...
def export_smpls_group(smpls_path, smpls_save_path):
    obj = bpy.data.objects[OBJ_NAME]
    arm_ob = bpy.data.objects[ARM_OBJ_NAME]
    scene = bpy.data.scenes['Scene']
    obname = "m_avg"
    if not os.path.exists(smpls_save_path): os.makedirs(smpls_save_path)
    driving_img_paths = [
        path for path in os.listdir(smpls_save_path + "/../images")
    ]
    driving_img_paths.sort(key=lambda x: int(x.split(".")[0]))
    smpl_file_lst = sorted(os.listdir(smpls_path))
    ...
    for frame in range(scene.frame_start, scene.frame_end):
        ...
        img_path = driving_img_paths[frame]
        img_fn, _ = os.path.splitext(os.path.basename(img_path))
        smpl_file = smpl_file_lst[frame]
        results_dict_for_rendering = np.load(f'{smpls_path}/{smpl_file}', allow_pickle=True).item()
        cam_result[1] = cam_result[1] * -1
        results_dict_for_rendering["cam_t"] = [cam_result]
        results_dict_for_rendering["smpls"] = smpl_result
        np.save(str(os.path.join(smpls_save_path, f"{img_fn}.npy")), results_dict_for_rendering)
...
if __name__ == "__main__":
    ...
    smooth()
    export_smpls_group(smpls_path, smpls_smoothed_path)
