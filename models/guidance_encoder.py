...
class GuidanceEncoder(ModelMixin):
    ...
    def forward(self, condition):
        ...
        embedding = F.silu(embedding) # [5, 16, 1, 768, 768]

        for block in self.blocks:
            embedding = block(embedding)
            embedding = F.silu(embedding) # [5, 32, 1, 384, 384] [5, 96, 1, 192, 192] [5, 256, 1, 96, 96]

        # FIXME: Temporarily only use the last attention.
        embedding = self.attentions[-1](embedding).sample # [5, 256, 1, 96, 96]
        embedding = self.conv_out(embedding) # [5, 320, 1, 96, 96]
