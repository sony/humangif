...
class UNet3DConditionModel(ModelMixin, ConfigMixin):
    ...
    def __init__(
        ...
        # Additional crossview 
        use_crossview_3d_attention=False,
        use_crossview_module=False,
        crossview_module_resolutions=(1, 2, 4, 8),
        crossview_module_mid_block=False,
        crossview_module_decoder_only=False,
        crossview_module_type=None,
        ...
        # if nerf_cond_type == 'add_latents':
        #     self.conv_in_feature_nerf = InflatedConv3d(
        #         32, block_out_channels[0], kernel_size=3, padding=(1, 1)
        #     )
        ...
        for i, down_block_type in enumerate(down_block_types):
            ...
            down_block = get_down_block(
                ...
                use_crossview_3d_attention=use_crossview_3d_attention,
                use_crossview_module=use_crossview_module
                and (res in crossview_module_resolutions)
                and (not crossview_module_decoder_only),
                crossview_module_type=crossview_module_type,
                crossview_module_kwargs=crossview_module_kwargs,
        ...
        if mid_block_type == "UNetMidBlock3DCrossAttn":
            self.mid_block = UNetMidBlock3DCrossAttn(
                ...
                use_inflated_groupnorm=use_inflated_groupnorm,
                use_crossview_3d_attention=use_crossview_3d_attention,
                use_crossview_module=use_crossview_module and crossview_module_mid_block,
                crossview_module_type=crossview_module_type,
                crossview_module_kwargs=crossview_module_kwargs,
    ...
    def forward(
        ...
        crossview_num: int = 4,
        feature_nerf_image: Optional[torch.Tensor] = None,
        ...
        # if feature_nerf_image is not None:
        #     feature_nerf_image_crop = feature_nerf_image[:, :, (feature_nerf_image.shape[-2] - feature_nerf_image.shape[-1])//2: (feature_nerf_image.shape[-2] + feature_nerf_image.shape[-1])//2]
        #     feature_nerf_image = self.conv_in_feature_nerf(feature_nerf_image_crop[:, :, None])
        #     sample = sample + feature_nerf_image
    ...
    def from_pretrained_2d(
        ...
        if view_module_path is not None:
            view_module_path = Path(view_module_path)
            # load the view module weights
            if view_module_path.exists() and view_module_path.is_file():
                if view_module_path.suffix.lower() in [".pth", ".pt", ".ckpt"]:
                    logger.info(f"Load view module params from {view_module_path}")
                    view_state_dict = torch.load(
                        view_module_path, map_location="cpu", weights_only=True
                    )
                elif view_module_path.suffix.lower() == ".safetensors":
                    view_state_dict = load_file(view_module_path, device="cpu")
                else:
                    raise RuntimeError(
                        f"unknown file format for view module weights: {view_module_path.suffix}"
                    )

            # merge the state dicts
            state_dict.update(view_state_dict)
