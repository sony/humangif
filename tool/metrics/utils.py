...
class DatasetFVDVideoFromFramesResize(torch.utils.data.Dataset):
    ...
    def __init__(self, files, sample_duration=16, mode='FVD-3DRN50', img_size=112, return_name=False):
        ...
        frame_format5 = r"^(\d+_)(\d+)(\.\w+)$"
        frame_format6 = r"^(\D+_\d_\d+_\d+_\D+\d+_)(\d+)(\.\w+)$"
        ...
        for file in files:
            ...
            elif re.match(frame_format5, file_name):
                folder_format_re = frame_format5
            elif re.match(frame_format6, file_name):
                folder_format_re = frame_format6
            else:
