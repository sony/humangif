...
pred_folder="${2:-${exp_folder}/pred_nv}"
gt_folder="${3:-${exp_folder}/gt_nv}"

echo "pred_folder" ${pred_folder}
echo "gt_folder" ${gt_folder}
# # L1 SSIM LPIPS and PSNR
python  tool/metrics/metric_center.py --root_dir /group2/3d/shoukanghu/champ --path_gen ${pred_folder}/ --path_gt ${gt_folder} --type l1 psnr ssim lpips --write_metric_to ${exp_folder}/metrics_nv_l1_ssim_lpips_psnr.json
...
# python  tool/metrics/metric_center.py --root_dir /group/3d/shoukanghu/champ --path_gen ${pred_folder}/ --path_gt ${gt_folder}/ --type fid-vid fvd  --write_metric_to ${exp_folder}/metrics_fid-vid_fvd.json --number_sample_frames 24 --sample_duration 24